...
let departs_possibles jeu joueur =
  ...
  let pion_de_ligne = List.filter (fun x -> x<> Vide) (List.map (fun x -> jeu (Case (x,i))) [1;2;3;4;5;6]) in
  ...
  List.filter (fun x -> (jeu x)<>Vide) (List.map (fun x-> Case (x,ligne)) [1;2;3;4;5;6])
;;
